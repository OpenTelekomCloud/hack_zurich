...
resource "null_resource" "provision-lamp" {
  depends_on = ["openstack_compute_floatingip_associate_v2.fip_2"]
...
resource "openstack_compute_floatingip_associate_v2" "fip_2" {
  count       = "${var.jumphost_count}"
  floating_ip = "${element(openstack_networking_floatingip_v2.fip.*.address, count.index)}"
  instance_id = "${element(openstack_compute_instance_v2.lamp.*.id, count.index)}"
}
